...
const downloadInformation = computed(() => ({
	preview: {
		...
		asset: (release.preview.assets ?? [])
			.find((a) => /^r\d{4,}.apk/.test(a.name)),
	},
	stable: {
		tagName: release.stable.tag_name ?? "v0.0.0",
		asset: (release.stable.assets ?? [])
			.find((a) => /libresudoku-v\d+\.\d+.\d+.apk|app-release.apk|libresudoku-arm64-v8a-v\d+\.\d+.\d+.apk/.test(a.name)),
	},
}))
...
<template>
	<div>
		<div v-if="!isAndroid" class="custom-block danger">
			...
			<p>
				<strong>LibreSudoku</strong> is an <strong>Android app</strong> only.
